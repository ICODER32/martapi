...
from db import create_db_and_tables,engine,Order,OrderCreate
import logging
...
@app.post("/order/")
async def create_order(order: OrderCreate,session=Depends(get_session), producer: AIOKafkaProducer = Depends(get_kafka_producer)):
    order=Order(
...
async def delete_order(order_id: int,session=Depends(get_session), producer: AIOKafkaProducer = Depends(get_kafka_producer)):
